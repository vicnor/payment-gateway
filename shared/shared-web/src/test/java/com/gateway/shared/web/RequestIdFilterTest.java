...
class RequestIdFilterTest {
  ...
  void eachRequestGetsUniqueId() throws Exception {
    String id1 =
        mockMvc.perform(get("/test/ping")).andReturn().getResponse().getHeader("X-Request-Id");
    String id2 =
        mockMvc.perform(get("/test/ping")).andReturn().getResponse().getHeader("X-Request-Id");
  ...
  void inboundXRequestIdIsIgnored() throws Exception {
    ...
    String returned =
        mockMvc
            .perform(get("/test/ping").header("X-Request-Id", inbound))
            .andReturn()
            .getResponse()
            .getHeader("X-Request-Id");

    assertNotNull(returned);
    assertNotEquals(inbound, returned, "Server must generate its own request-id, not echo inbound");
    assertTrue(returned.matches(REQ_ID_PATTERN), "Generated ID should match req_<ULID> pattern");
  }
}
